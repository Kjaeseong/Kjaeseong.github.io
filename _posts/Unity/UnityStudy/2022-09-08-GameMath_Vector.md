...
- 대수적으로 벡터가 동일한 차원이고 대응되는 성분이 같은 경우 동일한 것으로 본다.
  - Ex><br/>
    $u_x = v_x$ , $u_y = v_y$ , $u_z = v_z$ 일 때<br/>
    $(u_x, u_y, u_z) = (v_x, v_y, v_z)$<br/>
  
### 벡터의 크기 계산
- 기하학적으로 벡터의 크기는 방향을 가진 선분의 길이
- 벡터의 성분이 주어졌다면, 크기를 대수학적으로 계산할 수 있다.
  - Ex><br/>
  문제 : $u = (1, 2, 3)$과 $v = (1, 1)$ 벡터의 크기를 구하라.<br/>
  $||u|| = \sqrt {u_x^2 + u_y^2 + u_z^2} $<br/>
  답 : <br/>
  $||u|| = \sqrt {1^2+2^2+3^2} = \sqrt {1+4+9} = \sqrt {14}$<br/>
  $||v|| = \sqrt {1^2+1^2} = \sqrt 2$<br/>
